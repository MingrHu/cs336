...
def run_multihead_self_attention(
    ...
    """
    mut_attention = transformer.MR_multihead_self_attention(d_model,num_heads)
    weights = {
    ...
    mut_attention.load_state_dict(weights)
    return mut_attention.forward(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mut_attention_with_rope = transformer.MR_multihead_self_attention(d_model,num_heads,max_seq_len,theta)
    weights = {
        "q_proj.weight":q_proj_weight,
        "k_proj.weight":k_proj_weight,
        "v_proj.weight":v_proj_weight,
    ...
    mut_attention_with_rope.load_state_dict(weights)
    return mut_attention_with_rope.forward(in_features,token_positions)
...
def run_transformer_block(
    ...
    transformer_block = transformer.MR_transformer_block(d_model,num_heads,d_ff,max_seq_len,theta)
...
def run_transformer_lm(
    ...
    """
    transformer_lm = transformer.MR_transformer_lm(vocab_size,context_length,d_model,num_layers,num_heads,d_ff,rope_theta)
...
def run_load_checkpoint(
    ...
    """
    return helper.load_checkpoint(src,model,optimizer)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    bpe_ins = bpe.MR_BPE(input_path,vocab_size,special_tokens)
    bpe_ins.pre_process_text()
    bpe_ins.train_bpe()
    return (bpe_ins.get_vocab(),bpe_ins.get_merges())
